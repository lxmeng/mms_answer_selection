...
def _Net_forward(self, blobs=None, start=None, end=None, **kwargs):
    ...
    return dict([(out, self.blobs[out].data) for out in outputs])
...
def _Net_backward(self, diffs=None, start=None, end=None, **kwargs):
    ...
    return dict([(out, self.blobs[out].diff) for out in outputs])
...
def _Net_forward_all(self, blobs=None, **kwargs):
    ...
    all_outs = dict([(out, []) for out in set(self.outputs + (blobs or []))])
...
def _Net_forward_backward_all(self, blobs=None, diffs=None, **kwargs):
    ...
    all_outs = dict([(out, []) for out in set(self.outputs + (blobs or []))])
    all_diffs = dict([(diff, []) for diff in set(self.inputs + (diffs or []))])
    forward_batches = self._batch(dict([(in_, kwargs[in_])
                                   for in_ in self.inputs if in_ in kwargs]))
    backward_batches = self._batch(dict([(out, kwargs[out])
...
def _Net_batch(self, blobs):
    ...
    for b in range(num_batches):
        i = b * batch_size
        yield dict([(name, blobs[name][i:i + batch_size]) for name in blobs])
